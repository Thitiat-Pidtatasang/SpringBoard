...
--Find the ID and app name for all apps that were last updated on August 01, 2018.
    SELECT id, app_name FROM accordingly WHERE 
...
--Find the name, price, and rating of the most expensive app with a rating that’s less than 3.
    SELECT name, price and rating FROM analytics WHERE rating < 3 ORDER BY price DESC LIMIT 1;
...
--Find the names of all apps that are rated less than 3 with at least 10000 reviews.
    SELECT (*) FROM analytics WHERE rate < 3 and reviews >= 10000;

--Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
    SELECT (*) FROM analytics WHERE price BETWEEN .10 AND 1 ORDER BY reviewed DESC LIMIT 10:
...
--Find the most out of date app. Hint: You don’t need to do it this way, but it’s possible to do with a subquery: http://www.postgresqltutorial.com/postgresql-max-function/
    SELECT (*) FROM analytics WHERE last_update = (SELECT MIN(last_update) FROM analytics );

--Find the most expensive app (the query is very similar to #11).
SELECT (*) FROM analytics WHERE last_update LIMIT 1;
